...
void city_demolish(int index){
    ...
    // Displays another collision circle that indicates that a building was destroyed.
    uLCD.circle(city_record[index].x, city_record[index].y, city_record[index].height - 1, BUILDING_COLOR);
    int i = 1001;
    // Another loop that will allow the explosion to stay there for sometime, and then erases it.
    while (i) {
        if (i == 1) {
            uLCD.circle(city_record[index].x, city_record[index].y, city_record[index].height - 1, BACKGROUND_COLOR);
            draw_landscape();
        }
        i--;
    }
